...
def simulate_lorenz(z0, t, sigma=10., beta=8/3, rho=28.):
    ...
    """
    def f(z, t): return [sigma*(z[1] - z[0]), z[0] *
                         (rho - z[2]) - z[1], z[0]*z[1] - beta*z[2]]

    def df(z, dz, t): return [sigma*(dz[1] - dz[0]),
...
def generate_lorenz_data(ics, t, n_points, linear=True, normalization=None,
    ...
    for i in range(n_ics):
        for j in range(n_steps):
            ...
            dx[i, j] = modes[0]*dz[i, j, 0] + modes[1] * \
                dz[i, j, 1] + modes[2]*dz[i, j, 2]
            if not linear:
                dx[i, j] += modes[3]*3*(z[i, j, 0]**2)*dz[i, j, 0] + modes[4]*3*(
                    z[i, j, 1]**2)*dz[i, j, 1] + modes[5]*3*(z[i, j, 2]**2)*dz[i, j, 2]

            ddx[i, j] = modes[0]*ddz[i, j, 0] + modes[1] * \
                ddz[i, j, 1] + modes[2]*ddz[i, j, 2]
            if not linear:
                ddx[i, j] += modes[3]*(6*z[i, j, 0]*dz[i, j, 0]**2 + 3*(z[i, j, 0]**2)*ddz[i, j, 0]) \
                    + modes[4]*(6*z[i, j, 1]*dz[i, j, 1]**2 + 3*(z[i, j, 1]**2)*ddz[i, j, 1]) \
                    + modes[5]*(6*z[i, j, 2]*dz[i, j, 2]**2 +
